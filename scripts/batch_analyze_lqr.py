...
# CONFIG
# =========================
DATA_GLOB = "../data/lqr_run_*.csv"      # ajusta si tus nombres difieren
OUT_FIG_DIR = "../figures"
OUT_DOC_DIR = "../docs"
...
def save_figure(run_id, res):
    ...
    plt.title(f"Respuesta al escalón – LQR ({run_id})")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
...
def save_txt(run_id, csv_path, fig_path, res):
    ...
    lines = []
    lines.append(f"=== RESULTADOS LQR ({run_id}) ===\n")
...
summary_txt = os.path.join(OUT_DOC_DIR, "lqr_summary.txt")
with open(summary_txt, "w", encoding="utf-8") as f:
    f.write("=== RESUMEN GLOBAL LQR (todos los ensayos) ===\n\n")
